...
static void patchUpdate(Patch* patch)
{
    patch->a = patch->p11 - patch->p10 - patch->p01 + patch->p00;
    patch->b = patch->p10 - patch->p00;
    patch->c = patch->p01 - patch->p00;
    patch->d = patch->p00;
}

static void patchDraw(Patch* patch)
{
    vec4 col = {1.f, 0.f, 0.f, 1.f};
    dlDrawLine(&patch->p00, &patch->p01, &col);
    dlDrawLine(&patch->p10, &patch->p11, &col);
    dlDrawLine(&patch->p00, &patch->p10, &col);
    dlDrawLine(&patch->p01, &patch->p11, &col);
}


static bool bilinearCollision(const vec3* start, const vec3* end, Patch* patch, float* t, float* u, float* v, vec3* contactPos)
{
    /*
	See paper: SD Ramsey, K Potter, C Hanson; 'Ray Bilinear Patch Intersections'
    */
    vec4 blue = {0.f, 0.f, 1.f, 1.f};
    patchDraw(patch);
    dlDrawLine(start, end, &blue);

    vec3 dir;
    vecsub(&dir, end, start);

    float A1 = patch->a.x*dir.z - patch->a.z*dir.x;
    float B1 = patch->b.x*dir.z - patch->b.z*dir.x;
    float C1 = patch->c.x*dir.z - patch->c.z*dir.x;
    float D1 = (patch->d.x - start->x)*dir.z - (patch->d.z - start->z)*dir.x;
    
    float A2 = patch->a.y*dir.z - patch->a.z*dir.y;
    float B2 = patch->b.y*dir.z - patch->b.z*dir.y;
    float C2 = patch->c.y*dir.z - patch->c.z*dir.y;
    float D2 = (patch->d.y - start->y)*dir.z - (patch->d.z - start->z)*dir.y;

    /*
	Solve the quadratic function in v
	...
	where
	    ...
	    k1 = (A2*C1 - A1*C2)
	    k2 = (D1 - A1*D2 + b2*C1 - B1*C2)
	    k3 = (B2*D1 - B1*D2)
    */
	    
    float k1 = (A2*C1 - A1*C2);
    float k2 = (A2*D1 - A1*D2 + B2*C1 - B1*C2);
    float k3 = (B2*D1 - B1*D2);

    // There are upto two solutions
    float v1, v2;
    int numSolutions = quadratic(k1, k2, k3, &v1, &v2);
    printf("numsolutions = %d, v1 = %f v2 = %f\n", numSolutions, v1, v2);
	...
	{
	    vec3 p1 = (u1*v1)*patch->a + u1*patch->b + v1*patch->c + patch->d; 
	    vec3 d = p1 - *start;
	    *t = vecdot(&dir, &d) / vecdot(&dir, &dir);
	    *u = u1;
	    *v = v1;
	    *contactPos = p1;
	    return *t>0.f && *t<1.f;
	...
	if (u1>=0.f && u1<=1.f && v1>=0.f && v1<=1.f)
	{
	    p1 = (u1*v1)*patch->a + u1*patch->b + v1*patch->c + patch->d; 
	    vec3 d = p1 - *start;
	    t1 = vecdot(&dir, &d) / vecdot(&dir, &dir);
	}
	else
	{
	    t1 = FLOAT_MAX;
	}
	if (u2>=0.f && u2<=1.f && v2>=0.f && v2<=1.f)
	{
	    p2 = (u2*v2)*patch->a + u2*patch->b + v2*patch->c + patch->d; 
	    vec3 d = p2 - *start;
	    t2 = vecdot(&dir, &d) / vecdot(&dir, &dir);
	}
	else
	{
	    t2 = FLOAT_MAX;
	...
	printf("(2): u%f v%f u%f v%f (%f %f)\n", u1, v1, u2, v2, t1, t2);

	if (t1>0.f && t1<1.f && t1<t2)
	{
	    *t = t1;
	    *u = u1;
	    *v = v1;
...
void render()
{
	...
	Patch patch;
	vecset(&patch.p00, -1.f, -2.f, 0.0f+0.f*sinf(time));
	vecset(&patch.p01, +1.f, -1.3f, -0.0f+0.f*sinf(2.f*time));
	vecset(&patch.p10, -1.f, +1.f, 0.f*sinf(time));
	vecset(&patch.p11, +1.f, +1.f, -0.f+1.f*sinf(3.f*time));
	
	//vecset(&patch.p00, -1.f, -1.f, -1.f);
	//vecset(&patch.p01, +1.f, -1.f, 0.f);
	//vecset(&patch.p10, -1.f, +1.f, 0.f);
	//vecset(&patch.p11, +1.f, +1.f, 0.f);
	patchUpdate(&patch);
	...
	for (u32 i=0; i<10; i++)
	{
	    vec3 start = {x, +0.3f+0.2f*x, -1.f};
	    vec3 end = {x, -0.1f, +1.f};
	    float t, u, v;
	    vec3 pos;
	    if (bilinearCollision(&start, &end, &patch, &t, &u, &v, &pos))
	    {
		vec4 col = {1.f, 1.f, 0.f, 1.f};
		glColor3f(col.x, col.y, col.z);
		drawPoint3d(pos.x, pos.y, pos.z, 0.01f);
	    }
	    x = x + dx;
	}
    }
