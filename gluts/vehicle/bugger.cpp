...
static void vehicleSubTick(Chassis* c, float dt)
{
	...
	for (int i=0; i<numWheels; i++)
	{
		...
		vec3 pointVel = getPointVel(c, &s->worldDefaultPos);
	...
	for (int repeat=0; repeat<numIterations; repeat++)
	{
		for (int i=0; i<numWheels; i++)
		{
			...
			{
				float penetration = -vecdot(&w->pos, &s->hitNorm) - s->hitPlaneD - vecdot(&s->hitNorm, &w->vel) * dt;
			...
			if (1)	// Axis Error
			{
				...
				if (vecsizesq(&error)>0.001f)
				{
					...
					float denom = computeDenominator(1.f/c->mass, 1.f/c->inertia, &offset, &norm);
...
static void display()
{
	...
	if (g_step>g_step2)
	{
		vehicleSubTick(&s_chassis, subDt);
		g_step2++;
	}
